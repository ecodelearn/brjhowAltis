// ******************************************************************************************
// * This project is licensed under the GNU Affero GPL v3. Copyright © 2014 A3Wasteland.com *
// ******************************************************************************************
//	@file Name: FAR_lastResort.sqf
//	@file Author: AgentRev

private ["_hasCharge", "_hasSatchel", "_hasIEDlS", "_hasIEDlB", "_hasIEDuS", "_hasIEDuB", "_mineType", "_mine"];

_hasCharge = "DemoCharge_Remote_Mag" in magazines player;
_hasSatchel = "SatchelCharge_Remote_Mag" in magazines player;
_hasIEDlS = "IEDLandSmall_Remote_Mag" in magazines player;
_hasIEDlB = "IEDLandBig_Remote_Mag" in magazines player;
_hasIEDuS = "IEDUrbanSmall_Remote_Mag" in magazines player;
_hasIEDuB = "IEDUrbanBig_Remote_Mag" in magazines player;

if !(player getVariable ["performingDuty", false]) then
{
	if (_hasCharge || _hasSatchel || _hasIEDlS || _hasIEDlB || _hasIEDuS || _hasIEDuB) then
	{
		if (["Executar Allahu Akbar?", "", "Yes", "No"] call BIS_fnc_guiMessage) then
		{
			player setVariable ["performingDuty", true];
			playSound3D [call currMissionDir + "client\sounds\lastresort.wss", vehicle player, false, getPosASL player, 0.7, 1, 1000];
			sleep 1.0;
			switch (true) do
			{
				case (_hasCharge):
				{
					_mineType = "DemoCharge_F";
					player removeMagazine "DemoCharge_Remote_Mag";
				};
				case (_hasSatchel):
				{
					_mineType = "SatchelCharge_F";
					player removeMagazine "SatchelCharge_Remote_Mag";
				};
				case (_hasIEDlS):
				{
					_mineType = "IEDLandSmall_F";
					player removeMagazine "IEDLandSmall_Remote_Mag";
				};
				case (_hasIEDlB):
				{
					_mineType = "IEDLandBig_F";
					player removeMagazine "IEDLandBig_Remote_Mag";
				};
				case (_hasIEDuS):
				{
					_mineType = "IEDUrbanSmall_F";
					player removeMagazine "IEDUrbanSmall_Remote_Mag";
				};
				case (_hasIEDuB):
				{
					_mineType = "IEDUrbanBig_F";
					player removeMagazine "IEDUrbanBig_Remote_Mag";
				};
			};

			sleep 1.75;

			_mine = createMine [_mineType, ASLtoAGL ((getPosASL player) vectorAdd [0, 0, 0.5]), [], 0];
			_mine setDamage 1;

			if (damage player < 1) then // if check required to prevent "Killed" EH from getting triggered twice
			{
				player setVariable ["A3W_deathCause_local", ["suicide"]];
				player setDamage 1;
			};

			player setVariable ["performingDuty", nil];
		};
	}
	else
	{
		titleText ["Pegue um explosivo da proxima vez seu tolo!", "PLAIN", 0.5];
	};
};
